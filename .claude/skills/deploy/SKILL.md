...
3. If there are changes:
   ...
   b. Run `git diff --cached --stat` and `git diff --cached` to understand what changed.
   c. Generate a concise, meaningful commit message based on the diff. Use conventional format: `type(scope): description`. Common types: feat, fix, content, style, refactor, chore.
   d. Commit using:
      ```
...
# Remove old files (except .git-related)
cd "$DEPLOY_DIR"
...
If any command in this sequence fails, report the error and attempt to clean up the worktree before stopping.
