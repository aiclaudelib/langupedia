...
class JsonFileBackend {
  ...
  async query(opts) {
    ...
    // Filter by entry type ONLY when opts.type is an actual entry type keyword.
    // The bridge passes query-mode values like 'hybrid' / 'vector' which are
    // NOT entry types and must not exclude semantic entries.
    const ENTRY_TYPES = new Set(['semantic', 'episodic', 'procedural', 'working', 'cache']);
    if (opts?.type && ENTRY_TYPES.has(opts.type)) {
      results = results.filter(e => e.type === opts.type);
    }
    if (opts?.tags?.length) {
      results = results.filter(e => opts.tags.every(t => (e.tags || []).includes(t)));
    }
    if (typeof opts?.updatedAfter === 'number') {
      results = results.filter(e => (e.updatedAt || 0) >= opts.updatedAfter);
    }
