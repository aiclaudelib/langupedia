...
// ── MCP memory write (ADR-048 Phase 2) ──────────────────────────────────────
// Dual-write:
//   1. claude-flow CLI detached → sql.js+HNSW (visible via MCP memory_list)
//   2. JSON store at .claude-flow/data/auto-memory-store.json → AutoMemoryBridge
//      reads this on SessionEnd `sync` to project high-confidence entries into
//      ~/.claude/projects/<proj>/memory/MEMORY.md.
const AUTO_STORE_PATH = path.join(
  process.env.CLAUDE_PROJECT_DIR || process.cwd(),
  '.claude-flow', 'data', 'auto-memory-store.json'
);

function appendToAutoStore(entry) {
  try {
    fs.mkdirSync(path.dirname(AUTO_STORE_PATH), { recursive: true });
    let arr = [];
    if (fs.existsSync(AUTO_STORE_PATH)) {
      try { arr = JSON.parse(fs.readFileSync(AUTO_STORE_PATH, 'utf-8')) || []; } catch { arr = []; }
      if (!Array.isArray(arr)) arr = [];
    }
    arr.push(entry);
    // Cap at 5k entries to prevent unbounded growth
    if (arr.length > 5000) arr = arr.slice(-5000);
    fs.writeFileSync(AUTO_STORE_PATH, JSON.stringify(arr, null, 2), 'utf-8');
  } catch { /* non-fatal */ }
}

function storeInMcpMemory({ key, value, namespace = 'default', tags = [], confidence = 0.7, category = 'project-patterns' }) {
  if (!key || !value) return;
  const now = Date.now();
  const truncKey = String(key).slice(0, 200);
  const truncValue = String(value).slice(0, 4000);

  // 1. Async CLI write → sql.js+HNSW (MCP-visible, keeps caller's namespace/tags)
  try {
    ...
    if (tags.length) cliArgs.push('--tags', tags.join(','));
    const child = spawn('claude-flow', cliArgs, {
      detached: true,
      stdio: 'ignore',
      env: { ...process.env, CLAUDE_FLOW_QUIET: '1' },
    });
    child.unref();
  } catch { /* non-fatal */ }

  // 2. Sync append to JSON store in the format AutoMemoryBridge expects:
  //    namespace='learnings' + tag='insight' + category metadata + confidence.
  //    This gets projected into ~/.claude/projects/<proj>/memory/MEMORY.md
  //    on Stop hook via auto-memory-hook.mjs sync.
  appendToAutoStore({
    ...
    namespace: 'learnings',
    tags: ['insight', ...tags],
    metadata: { confidence, category, source: 'hook', originalNamespace: namespace },
    accessLevel: 'private',
    createdAt: now,
    updatedAt: now,
    version: 1,
...
const handlers = {
  ...
  'post-edit': () => {
    ...
    // ADR-048 Phase 2: persist edit insight into MCP-visible memory
    if (file) {
      const ext = path.extname(file).replace('.', '') || 'none';
      storeInMcpMemory({
        key: `edit:${path.basename(file)}:${Date.now()}`,
        value: `Edited ${path.relative(process.env.CLAUDE_PROJECT_DIR || process.cwd(), file)} via ${toolName || 'Edit'}`,
        namespace: 'edits',
        tags: ['post-edit', ext],
        confidence: 0.65,
  ...
  'post-task': () => {
    ...
    // ADR-048 Phase 2: persist task outcome into MCP-visible memory
    const agentType = hookInput.agentType || hookInput.subagent_type || process.env.CLAUDE_CODE_AGENT_NAME || 'main';
    const summary = (prompt && typeof prompt === 'string' ? prompt : '').slice(0, 500) || '(no summary)';
    storeInMcpMemory({
      key: `task:${agentType}:${Date.now()}`,
      value: `[${agentType}] ${summary}`,
      namespace: 'tasks',
      tags: ['post-task', agentType],
      confidence: 0.75,
      category: 'project-patterns',
    });
